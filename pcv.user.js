...
// @version      43.0
// @description  Foresight Plus: Foresight-only lane visualizer with safe Grafana display, compact allocations, Top Recirc, and no Grafana page injection.
// @author       janazare
...
(function () {
    ...
    const BUILD = {
        ...
        version: "43.0",
        author: "janazare",
        signature: "FORESIGHT-PLUS-JANAZARE-43"
    };
...
`);
    ...
    function isGrafanaPage() {
        // v43 safety: this script no longer runs on Grafana pages.
        return false;
    }
    ...
    function renderControlBar() {
        ...
        bar.innerHTML = `
            <span class="fcv-pro-left-controls">
            ...
            <span class="fcv-pro-right-controls">
        ...
        if (collapseToggle) {
            collapseToggle.addEventListener("click", toggleMasterCollapse);
        }
        ...
        if (refreshGrafana) {
            refreshGrafana.addEventListener("click", async () => {
                ...
                try {
                    if (typeof runGrafanaDiagnostics === "function") {
                        await runGrafanaDiagnostics();
                    }
                    renderAll();
                } finally {
                    refreshGrafana.textContent = "Refresh Grafana";
                    refreshGrafana.disabled = false;
                }
            });
        }
        ...
        if (refreshAllocations) {
            refreshAllocations.addEventListener("click", () => {
                ...
                try {
                    if (typeof refreshAllocationsHiddenFrame === "function") {
                        refreshAllocationsHiddenFrame();
                    } else {
                        extractAllocationsFromPage();
                    }
                } finally {
        ...
        const html = limited.length
            ? limited.map(item => `
                <div class="fcv-pro-item ${countClass(item.count)}-row" title="${escapeHtml(grafanaDisplayName(item.lane))}: ${item.count}">
                    <span class="fcv-pro-item-name">${escapeHtml(grafanaDisplayName(item.lane))}</span>
            ...
            const content = top.length
                ? top.map((lane, i) => `
                    <div class="fcv-pro-top-line ${severityClass(lane.recircValue)}" title="${escapeHtml(lane.laneLabel)}: ${lane.recircValue}">
                        <span class="fcv-pro-top-lane">${i + 1}. ${escapeHtml(lane.laneLabel)}</span>
    ...
    function ensureCreatorSignature() {
        ...
        if (!signature) {
            ...
            signature.textContent = "Foresight Plus v43 • @janazare";
...
`);
    ...
    GM_addStyle(`
/* Foresight Plus v43 version label + watermark */
